...
## Omit is used for remove a specific property from a type or interface.


```typescript
// Omit is used to remove specific property from a type or interface.
interface IUser {
    name: string,
    email: string,
    password: string,
...
interface IUserWithoutPasswordAndMobile extends Omit<IUser, "password">, Document {
    comparePassword: (password: string) => boolean
}


// Now we can use this interface to create a new type or interface
